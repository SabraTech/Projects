...
public class Factory {
  ...
  public Shape getShapeByExactName(String exactName) {
    String ret = exactName + "\n";
    for (int counter = 0; counter < shapes.size(); counter++) {
      ret += shapes.get(counter).getName() + "\n";
      if (shapes.get(counter).getName().equals(exactName)) {
        ...
        try {
          ...
          // TODO Auto-generated catch block
          throw new RuntimeException("at first catch " + ret);
        }
      }
    }
    try {
      // here I'm sure that the shape is not a plugin shape, so it's not loaded by URLClassLoader
