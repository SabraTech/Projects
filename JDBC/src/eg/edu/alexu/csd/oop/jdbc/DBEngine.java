...
public class DBEngine {
   ...
   * @throws SQLException
   */
  public boolean createDatabase(String databaseName, boolean dropIfExists) throws SQLException {
    ...
    if (file.exists() && dropIfExists) {
      ...
      throw new SQLException(databasesDirectory + "--" + databaseName + " =" + databasePath + file
          .exists());
    }
    return true;
  }
   ...
   * RuntimeException("unable to create database" + databasePath + " "); } }
   * else { currentDataBaseDirectory = databasePath; return true; } } else { if
   * (file.exists()) { if (file.isDirectory()) { // list of all files in it
   * String[] files = file.list(); for (String temp : files) { File dummyFile =
   * new File(file, temp); dummyFile.delete(); } } currentDataBaseDirectory =
   * databasePath; return true; } else { currentDataBaseDirectory =
   * databasePath; return true;
   */
  /*
