...
public class Engine implements Database {
  ...
  public boolean executeStructureQuery(String query) throws SQLException {
    ...
    if (parserAndValidator.queryIsCreateTable(query)) {
      TableCreationParameters creationParameters = parserAndValidator.getAddedTableParameters(
          query);
  ...
  public Object[][] executeQuery(String query) throws SQLException {
    ...
    } else if (parserAndValidator.isSelectAllWithCondition(query)) {
    ...
    throw new RuntimeException("unimplemented or error" + query);
  }
  ...
  public int executeUpdateQuery(String query) throws SQLException {
    if (parserAndValidator.queryIsInsertIntoTableColumnWithValues(query)) {
    ...
    } else if (parserAndValidator.queryIsConditionalDelete(query)) {
      ...
      for (int counter = 0; counter < columnsToChange.length; counter++) {
        for (int counter2 = 0; counter2 < columnsNames.length; counter2++) {
          if (columnsNames[counter].equalsIgnoreCase(columnsToChange[counter2])) {
            index[counter] = counter2;
          }
        }
        if (index[counter] == -1) {
